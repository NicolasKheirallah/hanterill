...
`npm run start` is gone: the site builds as a static export (`output: "export"`
in `next.config.ts`), so there is no server to start. Serve `out/` with any
static file server to preview the production build, e.g. `npx serve out`.
...
src/
  app/                 routes (App Router)
    page.tsx           "/" -> "/en" redirect (a static export has no middleware)
...
`src/lib/github.ts` follow. The Download section fetches the latest release from
the GitHub API **at build time** (a static export cannot revalidate) and renders
an offline fallback if GitHub cannot be reached, so a build never fails on the
API and the page never shows fabricated numbers. Re-run the build to refresh the
release data.

## Deploying to GitHub Pages

`.github/workflows/deploy.yml` builds the static export and publishes `out/` to
GitHub Pages on every push to `master` (rename the branch trigger if the default
branch differs). In the repository settings, set **Settings -> Pages -> Source**
to **GitHub Actions**.

- **Project site** (`https://<user>.github.io/<repo>/`): the workflow reads the
  base path from the Pages API and passes it as `NEXT_PUBLIC_BASE_PATH`, so
  `next.config.ts` prefixes every link and asset. Nothing to configure.
- **User/org site or a custom domain** (`opencma.org`): the base path is empty
  and the build serves from `/`. For a custom domain, add a `public/CNAME` file
  containing the domain and set it under Settings -> Pages.

For a local production preview: `npm run build && npx serve out`.
