...
const dtcStateName: Record<string, "stateActive" | "statePending" | "stateStored" | "stateHistorical"> = {
  active: "stateActive",
  ...
  return (
    <ul className="divide-y divide-line">
      {dtcDemo.map((d) => {
        ...
        const name = dtcStateName[d.state];
        return (
          <li key={d.code} className="py-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className={cn("text-[12px]", toneText[meta.tone])}>{td(name)}</span>
            </div>
            <p className="mt-0.5 text-[13px] text-text-secondary">{td(`faultTitles.${d.code}`)}</p>
...
export function LiveView() {
  const t = useTranslations("views");
  const tc = useTranslations("telemetry.channelNames");
  return (
    <div className="space-y-4">
      <MiniChart data={liveTrace(90)} min={392} max={404} unit="V" label={t("packVoltage")} height={110} live />
      <div className="grid grid-cols-2 gap-x-6">
        {liveChannels.map((c) => (
          <Field key={c.id} label={tc(c.id)} value={`${c.value} ${c.unit}`} />
        ))}
      </div>
    </div>
  );
...
export function LogsView() {
  const lines = [
    ...
    "12:04:18.402  DoIP  routing activation  accepted",
    "12:04:18.955  UDS   extended session  BECM",
    "12:04:19.140  UDS   read battery state of health  BECM",
    "12:04:19.302  UDS   read DTC information  all statuses",
    "12:04:20.001  UDS   read pack voltage  BECM",
